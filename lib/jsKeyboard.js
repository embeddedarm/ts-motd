var jsKeyboard = {
    ...
    init: function(elem, keyboard, fabricObject) {
        ...
        if (fabricObject) {
          jsKeyboard.currentElement = fabricObject;
          jsKeyboard.currentElementCursorPosition = fabricObject.get2DCursorLocation().charIndex;

          // Clear the default 'Edit me...' text so we can start writing our own thing.
          if (jsKeyboard.currentElement.text == 'Edit me...') {
              jsKeyboard.currentElement.selectAll();
              jsKeyboard.currentElement.removeChars('removeCharsEvent');
          }
        }
    ...
    {
        if (!jsKeyboard.currentElement.canvas) {
    ...
    write: function(m) {
        // This is a fabric object
        if (jsKeyboard.currentElement.canvas) {
            var c = String.fromCharCode(m);
            jsKeyboard.currentElement.insertChar(c);

            // For Debugging
            //window._currentElement = jsKeyboard.currentElement;
        }
        else {
    ...
    del: function() {
        if (jsKeyboard.currentElement.canvas) {
            if (jsKeyboard.currentElement.get2DCursorLocation().charIndex != 0) {
              jsKeyboard.currentElement.removeChars('removeCharsEvent');
            }
        }
        else {
    ...
    enter: function() {
        if (jsKeyboard.currentElement.canvas) {
            jsKeyboard.currentElement.insertChar("\n");
        }
        else {
    ...
    moveDirection: function(direction) {
        if (jsKeyboard.currentElement.canvas) {
            if (direction == "Up" || direction == "Down") {
                jsKeyboard.currentElement._moveCursorUpOrDown(direction, 'moveCursor' + direction);
            }
            else if (direction == "Right") {
                if (jsKeyboard.currentElement.get2DCursorLocation().charIndex < 
                    jsKeyboard.currentElement.text.length) {
                    console.log("Moving cursor to the right");
                    jsKeyboard.currentElement._moveCursorLeftOrRight(direction, 'moveCursorRight');
                }
            }
            else if (direction == "Left") {
                if (jsKeyboard.currentElement.get2DCursorLocation().charIndex > 0) {
                    console.log("Moving cursor to the left");
                    jsKeyboard.currentElement._moveCursorLeftOrRight(direction, 'moveCursorLeft');
                }
            }
        }
        else {
    ...
    hide: function() {
        ...
        // TODO:  Not sure what this is for.  Pulled from git stash.
        //if (jsKeyboard.currentElement.canvas) {
        //    jsKeyboard.currentElement.initBehavior()
        //}
    },
    defaultKeyboard: {
        capitalLetter:
        ...
        // 5th row
               { value: "&larr;", isChar: "false", onclick: "jsKeyboard.moveDirection('Left')", keyClass: "key" },
               { value: "&rarr;", isChar: "false", onclick: "jsKeyboard.moveDirection('Right')", keyClass: "key" },
               { value: "&uarr;", isChar: "false", onclick: "jsKeyboard.moveDirection('Up')", keyClass: "key" },
               { value: "&darr;", isChar: "false", onclick: "jsKeyboard.moveDirection('Down')", keyClass: "key" },
               { value: "<i class=\"fa fa-angle-double-down\" aria-hidden=\"true\"></i>", isChar: "false", onclick: "jsKeyboard.hide()", keyClass: "key" }
            ],
        smallLetter: [
        // 1st row
